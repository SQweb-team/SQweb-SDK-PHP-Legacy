 ...
 * <code>
 * $sqweb = new SQweb();
 * $sqweb = new SQweb(['SQW_ID_SITE'=>1234]);
 *
...
class SQweb
{
    ...
    private function loadConfig($config = array())
    {
        ...
        if (file_exists(__DIR__ . '/../../sqweb.config')) {
            $lines = file(dirname(__FILE__) . '/../../sqweb.config');
        ...
        if (empty($this->response)) {
            ...
            if (isset($_COOKIE['sqw_z']) && null !== $site_id) {
                ...
                curl_setopt_array($curl, array(
                    ...
                    CURLOPT_USERAGENT => 'SQweb/SDK Legacy 1.0',
   ...
   /*
    ...
    public function waitToDisplay($date, $wait = 0)
    {
        ...
        $format = 'Y-m-d';
        $today = date($format);
        $embargo = date($format, strtotime($today . ' + ' . (string)$wait . 'days'));
        $today = strtotime($today);
        $embargo = strtotime($embargo);
        return $today > $embargo ? true : false;
    }
}
